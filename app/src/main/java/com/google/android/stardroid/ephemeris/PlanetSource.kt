 ...
 * [com.google.android.stardroid.source.AstronomicalSource] for planets.
 *
 * @author Brent Bryan
 */
class PlanetSource(
    ...
    override val searchLocation = Vector3(0f, 0f, 0f)
    ...
    private fun updateCoords(time: Date) {
        ...
        searchLocation.updateFromRaDec(universe.getRaDec(planet, time))
    ...
    override fun initialize(): Sources? {
        ...
        if (planet === Planet.Moon) {
            imagePrimitives.add(
                ImagePrimitive(
                    searchLocation, resources, imageId, sunCoords,
            ...
            if (usePlanetaryImages || planet === Planet.Sun) {
                imagePrimitives.add(
                    ImagePrimitive(
                        searchLocation, resources, imageId, UP,
                        solarSystemObject.getPlanetaryImageSize()
                    )
                )
            } else {
                pointPrimitives.add(PointPrimitive(searchLocation, PLANET_COLOR, PLANET_SIZE))
            }
        }
        labelPrimitives.add(TextPrimitive(searchLocation, name, PLANET_LABEL_COLOR))
        return this
    }
