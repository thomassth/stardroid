...
public class AstronomerModelImpl implements AstronomerModel {
    ...
    private void calculateLocalNorthAndUpInPhoneCoordsFromSensors() {
        ...
        if (useRotationVector) {
            ...
            magneticNorthPhone = Geometry.addVectors(magneticFieldToNorth,
                    Geometry.scaleVector(down, -Geometry.scalarProduct(magneticFieldToNorth, down)));
            magneticNorthPhone.normalize();
            upPhone = Geometry.scaleVector(down, -1);
            magneticEastPhone = Geometry.vectorProduct(magneticNorthPhone, upPhone);
        }
